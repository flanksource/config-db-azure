...
func (s *Scraper) fetchAdvisorAnalysis() v1.ScrapeResults {
	if !s.config.Includes(IncludeAdvisor) {
		return nil
	}

	advisorClient, err := armadvisor.NewRecommendationsClient(s.config.SubscriptionID, s.cred, nil)
	if err != nil {
